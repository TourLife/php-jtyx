...
//        ]);
    ...
    private function share_money($id)
    {
        ...
        //一级佣金发放
        if ($this->share_setting->level > 1 || ($this->share_setting->level == 1 && $this->share_setting->is_rebate == 0)) {
        ...
        //二级佣金发放
        if ($this->share_setting->level > 2 || ($this->share_setting->level == 2 && $this->share_setting->is_rebate == 0)) {
        ...
        //三级佣金发放
        if ($this->share_setting->level >= 3 && $this->share_setting->is_rebate == 0) {
    ...
    private function transferToMch($e)
    {
        try {
            ...
            foreach ($order_list as $order) {
                ...
                $accountMoney = floatval($mch->account_money) + floatval($order->pay_price * (1 - floatval($mch->transfer_rate) / 1000)) - $order->share_price;
                $mch->account_money = $accountMoney;
                $mch->save();
                $order->is_transfer = 1;
                if (!$order->save()) {
