    ...
    public function shareOrderExportData($sql)
    {
        ...
        for ($i = 0; $i < $count; $i += $limit) {
            ...
            foreach ($data as $datum) {
                ...
                // 分销详情
                $share = isset($datum['share']) ? '一级分销商:' . $datum['share']['nickname'] . ',一级佣金:' . floatval($datum['first_price']) . '|' : '';
                $share1 = isset($datum['share_1']) ? '二级分销商:' . $datum['share']['nickname'] . ',二级佣金:' . floatval($datum['second_price']) . '|' : '';
                $share2 = isset($datum['share_2']) ? '三级分销商:' . $datum['share']['nickname'] . ',三级佣金:' . floatval($datum['third_price']) . '|' : '';
        ...
        foreach ($data as &$datum) {
            ...
            if ($datum->orderDetail) {
                foreach ($datum->orderDetail as $item) {
                    $costPriceCount += isset($item['cost_price']) ? $item['cost_price'] : 0;
                }
            }
