...
class ShareQrcodeForm extends ApiModel
{
    ...
    public function goods_qrcode()
    {
        ...
        if ($goods->is_negotiable) {
            $editor->text($goods_qrcode, '价格面议', 45, 30, 910, new Color('#ff4544'), $font_path, 0);
        } else {
...
//        $editor->blend($goods_qrcode, $wxapp_qrcode, 'normal', 1.0, 'top-left', 470, 1040);
    ...
    public function bargain_qrcode()
    {
        ...
        // 商品价格钱币符
        $editor->text($goods_qrcode, '最低￥', 20, 30, 1002, new Color('#ff5c5c'), $font_path, 0);
        //加商品价格
        $editor->text($goods_qrcode, $bargain->min_price, 34, 105, 990, new Color('#ff5c5c'), $font_path, 0);
        ...
        //倒计时时间
        $editor->text($goods_qrcode, $end_time, $name_size, 400, 770, new Color('#ffffff'), $font_path, 0);
        //保存图片
        $editor->save($goods_qrcode, $goods_pic_save_path . $goods_pic_save_name, 'jpeg', 85);
    ...
    public function topic_qrcode()
    {
        ...
        $editor->text($goods_qrcode, $read, 20, 40, 604, new Color('#919191'), $font_path, 0);
    ...
    public function step_qrcode()
    {
        ...
        if ($this->user) {
            ...
            $username = $this->setName($user->nickname);
            $namewitch = imagettfbbox(20, 0, $font_path, $username);
            $editor->text($goods_qrcode, $username, 20, 398, 960, new Color('#5b85cf'), $font_path, 0);
           
            $text = '已走了' . $this->num . '步';
            $textwitch = imagettfbbox(20, 0, $font_path, $text);

            $editor->text($goods_qrcode, $text, 20, 398, 995, new Color('#353535'), $font_path, 0);
            unlink($user_pic_path);
        }
        ...
        $editor->text($goods_qrcode, $qrcode_title, 25, 298, 1040, new Color('#353535'), $font_path, 0);
        $editor->text($goods_qrcode, '长按识别小程序码', 20, 298, 1093, new Color('#999999'), $font_path, 0);
